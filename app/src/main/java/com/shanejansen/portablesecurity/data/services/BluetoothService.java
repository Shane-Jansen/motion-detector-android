package com.shanejansen.portablesecurity.data.services;
...
import com.shanejansen.portablesecurity.data.helpers.BluetoothConnectedHelper;
import com.shanejansen.portablesecurity.ui.main.activities.MainActivity;
...
public class BluetoothService extends Service {
  // Constants
  private static final String ACTION_WAIT_PRESSED =
      "com.sjjapps.BluetoothService.ACTION_WAIT_PRESSED";
  private static final String ACTION_STOP_PRESSED =
      "com.sjjapps.BluetoothService.ACTION_STOP_PRESSED";
  private static final String ACTION_MOTION_DETECTED_PRESSED =
      "com.sjjapps.BluetoothService.ACTION_MOTION_DETECTED_PRESSED";
  private static final String ACTION_DISCONNECTED_CONFIRMED_PRESSED =
      "com.sjjapps.BluetoothService.ACTION_DISCONNECTED_CONFIRMED_PRESSED";
  ...
  private BroadcastReceiver mWaitReceiver, mStopReceiver, mMotionDetectedReceiver,
      ...
      @Override public boolean handleMessage(Message msg) {
        switch (msg.what) {
          case BluetoothConnectedHelper.HANDLER_RECEIVED_DATA:
            ...
            // Wake the screen
            PowerManager pm =
                (PowerManager) BluetoothService.this.getSystemService(Context.POWER_SERVICE);
  ...
  private void showOngoingNotification() {
    ...
    PendingIntent pendingIntentWait =
        PendingIntent.getBroadcast(this, 123, waitReceive, PendingIntent.FLAG_UPDATE_CURRENT);
    PendingIntent pendingIntentStop =
        PendingIntent.getBroadcast(this, 123, stopReceive, PendingIntent.FLAG_UPDATE_CURRENT);
  ...
  private void showMotionDetectedNotification() {
    ...
    PendingIntent pendingIntentMotion =
        PendingIntent.getBroadcast(this, 123, motionReceive, PendingIntent.FLAG_UPDATE_CURRENT);
  ...
  private void showDisconnectedNotification() {
    ...
    registerReceiver(mDisconnectedReceiver,
        new IntentFilter(ACTION_DISCONNECTED_CONFIRMED_PRESSED));

    // Create intents for pressing notification buttons
    Intent disconnectedReceive = new Intent(ACTION_DISCONNECTED_CONFIRMED_PRESSED);
    PendingIntent pendingIntentMotion = PendingIntent.getBroadcast(this, 123, disconnectedReceive,
  ...
  private void bluetoothFinished() {
    if (mConnectedHelper.isConnected()) mConnectedHelper.closeConnection();
    if (mWaitReceiver != null) unregisterReceiver(mWaitReceiver);
    if (mStopReceiver != null) unregisterReceiver(mStopReceiver);
    if (mMotionDetectedReceiver != null) unregisterReceiver(mMotionDetectedReceiver);
    if (mDisconnectedReceiver != null) unregisterReceiver(mDisconnectedReceiver);
